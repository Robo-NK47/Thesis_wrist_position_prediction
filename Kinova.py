...
def example_move():
    ...
    while not target_reached:
        ...
        if (isclose(tcp_x, TARGET_1_X, abs_tol=TRANSLATION_TOLERENCE) and isclose(tcp_y, TARGET_1_Y,
                ...
                isclose(tcp_theta_y, TARGET_1_THETA_Y, abs_tol=ORIENTATION_TOLERENCE) and
                isclose(tcp_theta_z, TARGET_1_THETA_Z, abs_tol=ORIENTATION_TOLERENCE)):
            target_reached = True
...
def main():
    ...
    while True:
        data_from_arduino = s.recv(1024)
        try:
            data_from_arduino = pickle.loads(data_from_arduino)
            last_good_data_from_arduino = data_from_arduino
        except pickle.UnpicklingError:
            data_from_arduino = last_good_data_from_arduino
