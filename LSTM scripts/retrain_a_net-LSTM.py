...
import openpyxl
import joblib
from data_class_LSTM_CNN import DataSet, DataSample, ToTensor, LSTM_CNN
...
def get_dataset(batch_size):
    main_path = r'/home/nadavk/IMU/Raw data/models/LSTM/Study id - 1640889453 - Nadav net - 60 steps ALL SENSORS yes MLP/---Datasets/'
    ####################### TRAIN DATASET #######################
            ...
            if cell[1:5] == selected_cell:
                for time_frame in data[epoch][cell]:
                    if len(data[epoch][cell][time_frame]['ground_truth']) > 0:
                        current_ground_truth = data[epoch][cell][time_frame]['ground_truth'][0]
                        current_prediction = data[epoch][cell][time_frame]['prediction'][0]

                        for ground, pred in zip(current_ground_truth.values(),
                                                current_prediction.values()):
                            real_x.append(ground[0].item())
                            real_y.append(ground[1].item())
                            real_z.append(ground[2].item())

                            predicted_x.append(pred[0].item())
                            predicted_y.append(pred[1].item())
                            predicted_z.append(pred[2].item())
...
def excel(path, action, data):
    if action == 'create':
        ...
        ws = wb.active
        headers = ['#', 'Loss type', 'Optimizer', 'LR', 'Epochs', 'Weight decay', 'Best train loss',
    ...
    if action == 'update':
        ...
        dat = [data['trial'].number, data['trial'].params['loss type'],
...
def objective(trial):
    ...
    # Load the model.
    model_dir = r'/home/nadavk/IMU/Raw data/models/LSTM/Study id - 1640889453 - Nadav net - 60 steps ALL SENSORS yes MLP/Model #32/'
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        var_condition = variance_list[-1]['variance'] < 1e-5
    ...
    del (EPOCHS, all_results, batch_idx, batch_size, best_trial_score, criterion, data, dataset_settings,
         ...
         sample_indexes, target, test_dataset, test_loss, test_losses, train_dataset, train_loss, train_losses,
...
if __name__ == "__main__":
    ...
    for _ in range(studies):
        ...
        trial_start = f'Study id - {study_num} - Nadav net - RETRAIN - {input("What type of dataset is this? ")}'
        ...
        if operating_system == 'linux':
            study_path = os.path.join(r'/home/nadavk/IMU/Raw data/models/LSTM/', trial_start)
